...
typedef struct {
   int current_desktop, previous_desktop;
   int wh, ww, wx, wy;
    ...
    for (client *c; nm<MONITORS-1;) {
        for (select_monitor(++nm), nd=-1, cd=current_desktop; nd<DESKTOPS-1;) {
            ...
            fprintf(stdout, "%d:%d:%d:%d:%d:%d:%d%c", nm, current_monitor == cm, nd, n, mode,
                    current_desktop == cd, urgent, (nm==MONITORS-1 && nd==DESKTOPS-1)?'\n':' ');
        }
...
void maprequest(XEvent *e) {
    ...
    if (cm != newmon) { select_monitor(newmon); cd = current_desktop; }
    ...
    if (cm != newmon) { select_monitor(cm); if (cd == newdsk) update_current(current); }
            ...
            case MotionNotify:
                ...
                if (arg->i == RESIZE) XResizeWindow(dis, current->win,
                   xw>MINWSZ ? xw:wa.width, yh>MINWSZ ? yh:wa.height);
                    ...
                    if ((monitor = areatomonitor(xw, yh)) != current->monitor) {
                        client *old = current;
...
void moveresize(const Arg *arg) {
    ...
    if (!current->isfloating && !current->istransient) { current->isfloating = True; tile(); }
    if ((monitor = areatomonitor(wa.x + ((int *)arg->v)[0], wa.y + ((int *)arg->v)[1])) != current->monitor) {
       client *old = current;
       client_to_monitor(&(Arg){.i = monitor});
       select_monitor(monitor);
       update_current(old);
...
void select_monitor(int i) {
    ...
    if (current) {
       XSetWindowBorder(dis, current->win, win_infocus);
       if (CLICK_TO_FOCUS) XGrabButton(dis, Button1, None, current->win, True,
             ButtonPressMask, GrabModeAsync, GrabModeAsync, None, None);
    }

    desktops         = monitors[i].desktops;
    current_desktop  = monitors[i].current_desktop;
    previous_desktop = monitors[i].previous_desktop;
    ww               = monitors[i].ww;
    wh               = monitors[i].wh;
    wx               = monitors[i].wx;
    wy               = monitors[i].wy;
...
void client_to_monitor(const Arg *arg) {
    XWindowAttributes wa;
    if (!current || arg->i == current->monitor ||
        !XGetWindowAttributes(dis, current->win, &wa)) return;
