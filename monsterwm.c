...
static void (*events[LASTEvent])(XEvent *e) = {
    ...
    [ConfigureRequest] = configurerequest, [MotionNotify] = motionnotify,
};
...
void desktopinfo(void) {
    client *c;
    Bool urgent = False;
    int m = 0, cm = current_monitor, cd = 0, n=0, d=0;
    for (; m<MONITORS; m++) {
...
void moveresize(const Arg *arg) {
    ...
    XMVRSZ(dis, current->win, wa.x + ((int *)arg->v)[0], wa.y + ((int *)arg->v)[1],
...
client* wintoclient(Window w) {
    client *c = NULL;
    Bool found = False;
    int m = 0, cm = current_monitor, d = 0, cd = 0;
    for (; m<MONITORS; ++m) {
        for (select_monitor(m), d = 0, cd = current_desktop; d<DESKTOPS && !found; ++d)
